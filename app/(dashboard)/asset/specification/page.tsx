...
import { assetSpecIndex } from "@/data/asset-spec"
...
const RenderTable = async ({
  ...
  const result = await assetSpecIndex(currentPage, size, query)
...
export default async function AssetSpecPage({
  searchParams,
}: {
  ...
  return (
    <>
      <div className="mb-4 flex items-center justify-between">
        <h2 className="mb-4 text-3xl font-bold">Asset Specification</h2>

        <Link href="/asset/specification/new" className={buttonVariants()}>
          Add New
        </Link>
      </div>

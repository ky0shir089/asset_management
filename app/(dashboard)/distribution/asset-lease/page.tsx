...
import { assetLeaseIndex } from "@/data/asset-lease"
...
const RenderTable = async ({
  ...
  const { data, meta } = await assetLeaseIndex(currentPage, size, query)

  return <DataTable columns={columns} data={data} meta={meta} />
}

export default async function AssetLeasePage({
  searchParams,
}: {
  ...
  return (
    <>
      <div className="mb-4 flex items-center justify-between">
        <h1 className="mb-4 text-3xl font-bold">Asset Lease</h1>

        <Link href="/distribution/asset-lease/new" className={buttonVariants()}>
          Add New
        </Link>
      </div>
